...
- PCB substrate and light soldermask;
- copper pour, traces, pads, vias, and dark silkscreen designators;
- DIP ICs (`U1`, `U2`), QFP (`U3`), film capacitors (`C1`, `C2`), axial resistor
  (`R1`), diode (`D1`), electrolytic cans (`E1`, `E2`), and headers (`H1`, `H2`);
- red signal, blue ground, and gate flying leads with slack;
...
check reports 11 footprints, 9 panel keep-outs, no overhangs, and no keep-out
intrusions.

## Optics and rig

- Polycarbonate IOR: `1.585`.
- Shell tint: `ShaderNodeVolumeAbsorption`, density `0.0042`; tint follows path
...
- One rig-level gain (`RIG_GAIN = 0.72`) drives key, fill, kicker, rear rim, and
...
| Probe | sRGB |
|---|---:|
| shell over interior | 154.3 |
| plated hardware | 107.5 |
| shell edge | 114.3 |
| darkest print pixel (`min`) | 63.0 |
| frosted print ground | 160.3 |
...
- the logo and preset hardware carriers, which are physical opaque inserts;
