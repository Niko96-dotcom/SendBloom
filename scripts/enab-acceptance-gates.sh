...
ctest --test-dir "$BUILD" -C Release --output-on-failure -R \
  "HF ringing (ProperSRC|no narrowband)|ProperSRC HF metrics invariant|fixed-rate reverb|Plugin reports the live ProperSRC latency|Engaged and bypass direct routes emerge at the reported PDC latency|14825 Hz imaging vs LegacyAccumulator"
