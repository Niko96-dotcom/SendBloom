...
TEST_CASE ("Dry path unchanged when gate closes wet", "[gate][DryNever][GatedBloomChain]")
{
    ...
    for (int i = 0; i < 15000; ++i)
    {
        ...
        chain.processSample (0.5f, env, 1.0f, 0.0f, 0.0f, 1.0f, false, kThresholdDb);
    }

    for (int i = 0; i < 4800; ++i)
    {
        const auto env = chain.getEnvelope().process (0.0f);
        const auto wet = chain.processSample (0.0f, env, 1.0f, 0.0f, 0.0f, 1.0f, false, kThresholdDb);
...
TEST_CASE ("Pre gate affects wet only not dry tap", "[gate][DryNever][GatedBloomChain]")
{
    ...
    {
        const auto env = chain.getEnvelope().process (0.0f);
        const auto wet = chain.processSample (0.0f, env, 1.0f, 0.0f, 0.0f, 1.0f, true, kThresholdDb);
